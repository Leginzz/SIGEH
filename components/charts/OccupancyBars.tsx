      ...
      <div className="grid grid-cols-4 sm:grid-cols-8 gap-1.5">
        {sorted.map(room => (
          <div key={room.id} className="flex flex-col items-center">
            <div className="text-xs font-bold text-slate-400 mb-1">#{room.id}</div>
            <div
              className="w-full rounded-lg transition-all duration-300"
              style={{
                backgroundColor: statusColor[room.status] || '#475569',
                height: room.status === 'Ocupada' ? '48px' : room.status === 'Reservada' ? '36px' : '24px',
                opacity: room.status === 'Disponible' ? 0.6 : 1,
              }}
              title={`Habitación ${room.id}: ${room.status}`}
            />
            <div className="text-[10px] text-slate-500 mt-0.5">{statusLabel[room.status] || room.status}</div>
          </div>
        ))}
      </div>
